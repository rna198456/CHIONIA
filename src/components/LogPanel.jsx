...
export default function LogPanel({ onClose }) {
  ...
  const visible = filter === "recent"
    ? entries.filter((e) => new Date(e.ts).getTime() > oneDayAgo)
    : entries;
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div>
            <div style={{ fontWeight: 600, fontSize: 15, color: "#f0ece0" }}>📊 Registro de interacciones</div>
            <div style={{ fontSize: 11, color: C.muted, marginTop: 2 }}>
              Panel docente · Chion Bot · localStorage
            </div>
          </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", borderBottom: `1px solid ${C.border}`, flexShrink: 0 }}>
          {[
            ...
            { label: "Prom/sesión", value: loading || !sessions ? "—" : (total/sessions).toFixed(1), color: "#a78bfa" },
          ].map((s, i) => (
        ...
        {/* Filtros */}
        <div style={{ padding: "10px 16px", borderBottom: `1px solid ${C.border}`, display: "flex", gap: 6, alignItems: "center", flexShrink: 0 }}>
          ...
          <div style={{ marginLeft: "auto", fontSize: 10, color: "#333" }}>
            ⚠ Registro local — exportá CSV para conservarlo
          </div>
        </div>
        ...
        <div style={{ flex: 1, overflowY: "auto" }}>
          {visible.length === 0 ? (
            <div style={{ padding: 40, textAlign: "center", color: C.muted }}>
              {total === 0 ? "Aún no hay consultas registradas." : "Sin registros con este filtro."}
            </div>
          ) : (
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
              ...
              <tbody>
                {visible.map((e, i) => (
                  <tr key={i} style={{ borderBottom: `1px solid #1a1a1a`, background: i%2===0?"transparent":"#191919" }}>
                    <td style={{ padding: "9px 14px", color: C.textDim, whiteSpace: "nowrap", fontSize: 11 }}>{fmt(e.ts)}</td>
                    <td style={{ padding: "9px 14px", fontFamily: "monospace", color: C.blue, fontSize: 11 }}>{e.sid}</td>
        ...
        <div style={{
          ...
          <span style={{ fontSize: 10, color: "#333" }}>Sin datos personales almacenados · Código de sesión aleatorio por visita</span>
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={() => exportCSV(entries)} disabled={!total} style={{
              fontSize: 11, padding: "7px 14px", background: C.card,
              border: `1px solid ${C.border}`, borderRadius: 8,
              color: total ? C.text : C.muted, cursor: total?"pointer":"default", fontFamily: "inherit",
            }}>⬇ Exportar CSV</button>
            <button onClick={handleClear} disabled={!total||clearing} style={{
            ...
            }}>{clearing ? "Limpiando…" : "🗑 Limpiar"}</button>
          </div>
        </div>
      </div>
    </div>
